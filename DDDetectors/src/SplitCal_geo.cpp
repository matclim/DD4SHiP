...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)  {
  ...
  const int num_x   =  x_widebar.attr<unsigned>("num_x");
  Box   widebar(x_widebar.x()-tol, x_widebar.y()-tol,x_widebar.z()-tol);
  Box   thinbar(x_thinbar.x()-tol, x_thinbar.y()-tol,x_thinbar.z()-tol);
...
//  box_vol.setVisAttributes(description.visAttributes(""));
  ...
  printout(INFO, "SandwichCalo", "%s: Layer:   nx: %7d x spacing: %7.3f", nam.c_str(), num_x, widebar_x_spacing);
//  Rotation3D rot(RotationZYX(0e0, 0e0, M_PI/2e0));
  ...
  for( int iz=0; iz < num_z; ++iz )  {
    ...
    if(iz==0){
    	    //Move reference for first placement
	    if((static_cast<int>(calo_layer_codes[0]-'0') == 1) || (static_cast<int>(calo_layer_codes[0]-'0') == 2)) z_layer += x_passive_layer.z()+x_widebar.z();
	    if((static_cast<int>(calo_layer_codes[0]-'0') == 3) || (static_cast<int>(calo_layer_codes[0]-'0') == 4)) z_layer += x_passive_layer.z()+x_thinbar.z();
